...
struct Map(HashMap<Coords, char>);
...
fn display_map(exploded_map: &Map, exploded_region: &Region, fences: &HashSet<Coords>) {
...
",
        ...
        display_map(&exploded_map, &regions[0], &fences);

        assert_eq!(
            fences,
            HashSet::from_iter(vec![(1, 0), (1, 2), (0, 1), (2, 1)])
        );
        display_map(&map, &regions[0], &fences);
    }
...
fn get_sides(exploded_map: &Map, exploded_region: &Region) -> usize {
    ...
    // display_map(exploded_map, exploded_region, &fences);
